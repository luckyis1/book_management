...
public class MemberDAO {
	...
	public void insertMember(MemberVO vo) {
		// String sql="insert into tbl_member(id, pwd, name, email,jumin)
		// values(?,?,?,?,?)";
	...
	public List<MemberVO> selectAll(){// 제네릭
		// 하나의 클래스로 여러가지의 타입을 다룰수 있도록 하는 애가 제네릭 MemberList=멤버VO만 저장 하고 관리한다 X
		// List 모든 타입을 리스트로 관리한다
	...
	public void updateMember(MemberVO vo){
		// String sql = "update tbl_member set pwd = ?,email=?, updatedate =
		// now() where id= ? ";
		// Connection con = dataSource.getConnection();
		// PreparedStatement pstmt = con.prepareStatement(sql);
